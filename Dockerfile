...
LABEL org.opencontainers.image.title="nats-sinks" \
      org.opencontainers.image.description="Oracle Linux based local testing image for nats-sinks JetStream sink runners." \
...
RUN microdnf install -y python3.11 python3.11-pip shadow-utils ca-certificates \
    && microdnf clean all \
    && python3.11 -m pip install --no-cache-dir ".${NATS_SINKS_EXTRAS:+[$NATS_SINKS_EXTRAS]}" \
    && python3.11 -m pip check \
    && groupadd --system nats-sinks \
    && useradd --system --gid nats-sinks --home-dir /var/lib/nats-sinks --shell /sbin/nologin nats-sinks \
