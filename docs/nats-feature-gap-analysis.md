...
| WebSocket connections | NATS URLs can use `ws://` for WebSocket connections, and NATS server WebSocket support can run alongside traditional TCP connections. | Evaluated in [WebSocket Connection Evaluation](websocket-connection-evaluation.md). `ws://` and `wss://` are accepted by URL validation, but production certification is split into configuration guardrails, optional header support, and integration certification backlog items. | Phase 3 |
