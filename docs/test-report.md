...
connector remains disabled by default, uses the shared allow and deny policy,
emits one bounded Splunk HEC metric event, reads HEC tokens and optional
headers from environment variables, enforces TLS verification, supports stale
snapshot guardrails and dry-run rendering through `nats-sink-observe
splunk-hec-export`, and keeps external Splunk delivery outside the NATS
worker. Focused Splunk HEC observability and public API tests passed with
`51 passed`. The full local validation suite also passed through
`scripts/check.sh`, including formatting for `189` files, linting, type
checking with no issues in `74` source files, the main pytest suite with
`805 passed, 9 skipped`, the encryption and sink contract suite with
...
This refresh covered issue `#106`, adding a Grafana Alloy profile over the
shared OTLP observability core. The profile remains disabled by default, uses
the shared allow and deny policy, emits low-cardinality Alloy profile resource
hints, sources local receiver headers from environment variables, generates
sanitized Alloy River configuration snippets, supports stale snapshot guardrails
and dry-run rendering through `nats-sink-observe grafana-alloy-export`, and
keeps Alloy forwarding outside the delivery worker. Focused Alloy
observability tests passed with `56 passed`. The full local validation suite
also passed through `scripts/check.sh`, including formatting for `187` files,
linting, type checking with no issues in `73` source files, the main pytest
suite with `789 passed, 9 skipped`, the sink capability suite with `104 passed`,
MkDocs builds for Read the Docs and GitHub Pages, security scanning,
package build, SBOM generation, checksum generation, and Twine metadata
validation. The skipped tests remain gated live NATS and Oracle integration
tests that require explicit local environment flags or services.

This refresh also preserves issue `#105`, adding an Elastic Observability
profile over the shared OTLP observability core. The profile remains disabled
by default, uses the shared allow and deny policy, emits Elastic-safe static
data stream routing hints, sources headers from environment variables, supports
stale snapshot guardrails and dry-run rendering through `nats-sink-observe
elastic-export`, and keeps direct Elasticsearch writes out of scope.
...
The current workspace is prepared on a workflow branch from `release-v0.4.1`
after the published `0.4.0` release and has passed local validation. It
includes:

- guarded non-main pull request auto-approval tooling for issue, feature, and
  bug branch PRs raised by the local workflow. The helper refuses PRs targeting
  `main`, can require the expected PR author, is integrated into
  `scripts/open-release-pr.sh --ready`, supports opt-out for manual
...
- pull request label synchronization for managed issue, feature, and bug
  ...
  names, removes stale project-managed PR labels while preserving manual
  labels, and leaves the official GitHub Issue Priority field on the issue
  rather than translating it into a PR label. The change is tracked in issue
  `#209`. Development testing found and fixed eight workflow bugs: issue
  `#211` changed label application from `gh pr edit` to `gh issue edit`;
  issue `#212` stopped Markdown code-span placeholders from being treated as
  real source issue references; issues `#213` through `#218` hardened
  existing-PR refresh, explicit `--issue` body rendering, source issue
  detection, dry-run wording, malformed GitHub JSON handling, and stale
  managed-label cleanup. Full local validation after these workflow changes
  passed with `691 passed, 8 skipped` in the main pytest run, `121 passed` in
...
| Encryption and sink contract suite | `scripts/check-encryption.sh` through `scripts/check.sh` | Pass | 123 encryption-focused, commit-then-ACK, file sink, and Oracle sink contract tests passed with generated temporary AES-256 key material that was deleted after the run |
| Sink capability suite | `scripts/check-sinks.sh` | Pass | 104 sink-focused tests passed plus file, encrypted file, and Oracle CLI smoke checks |
