...
"""Map NATS envelopes to Oracle MySQL row values.

Oracle MySQL stores JSON-capable fields using the same payload-normalization
contract as Oracle Database and the file sink.  Valid JSON is preserved, while
opaque text or encrypted bytes can be wrapped in the standard nats-sinks JSON
envelope.  The mapper does not open database connections and does not know
table names; it only produces typed row dictionaries for SQL binding.
"""
...
from nats_sinks.mysql.idempotency import validate_envelope_idempotency
...
def envelope_to_row(
    ...
    *,
    ...
    """Convert one normalized envelope into Oracle MySQL bind values."""
...
def envelopes_to_rows(
    ...
    *,
    ...
    """Convert a batch of envelopes into Oracle MySQL bind rows."""
